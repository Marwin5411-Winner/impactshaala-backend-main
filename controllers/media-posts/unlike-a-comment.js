...
async function unlikeCommentedPost(req, res, next) {
        ...
        for(let i = 0; i < mediaPost.comments.length; i++){
            ...
            if(comment._id.toString()===commentId.toString()){
                if(comment.likes.includes(profile._id)){
                    comment.likes.remove({_id:profile._id});
                    await mediaPost.save();
                    break;
                }else{
                    return res.status(400).json({success:false,message:"You have removed your like from this comment."});
                }
    
            }
        }

        return res.status(200).json({success:true,message:"You have removed your like from this comment."});
...
export default unlikeCommentedPost;